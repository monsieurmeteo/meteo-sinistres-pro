...
export default function SinistreMap({ sinistre, stations = [] }) {
  if (!sinistre || typeof sinistre.lat !== 'number' || typeof sinistre.lon !== 'number') {
    return (
      <div className="h-72 rounded-xl bg-slate-100 flex items-center justify-center border border-slate-200 text-slate-500 text-xs font-medium">
  ...
  return (
    <div className="w-full space-y-2">
      <div 
        ...
        className="w-full aspect-[3/2] min-h-[300px] max-h-[360px] rounded-xl overflow-hidden border border-slate-200 shadow-sm relative bg-slate-100"
      >
        <MapContainer
          key={mapKey}
          center={center}
      ...
      {/* Écritures parfaitement contrastées et lisibles en mode clair */}
      <div className="flex flex-wrap items-center justify-between px-1 text-xs pt-1">
        <span className="flex items-center gap-1.5 text-slate-700 font-medium">
          ...
          <strong className="text-slate-900 font-bold">Lieu du sinistre :</strong> {sinistre.adresseSinistre || sinistre.commune}
        </span>
        <span className="text-sky-800 font-mono text-[11.5px] font-bold">
