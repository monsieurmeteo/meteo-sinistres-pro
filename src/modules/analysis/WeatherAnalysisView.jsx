...
export default function WeatherAnalysisView({ dossier, onBack, onUpdateDossier }) {
  ...
  if (loading) {
    return (
      <div className="rounded-3xl p-12 text-center border-2 border-slate-200 bg-white max-w-xl mx-auto my-12 shadow-sm">
        <RefreshCw className="w-9 h-9 text-sky-600 animate-spin mx-auto mb-4" />
        <h2 className="text-lg font-black text-slate-950 mb-1">Expertise Météorologique en cours</h2>
        <p className="text-xs text-slate-600 font-mono animate-pulse">{progressMsg || 'Chargement des 5 stations officielles Météo-France…'}</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-3xl p-8 border-2 border-rose-300 bg-rose-50 max-w-xl mx-auto my-12 text-center shadow-sm">
        <AlertCircle className="w-9 h-9 text-rose-600 mx-auto mb-3" />
        <h2 className="text-base font-black text-rose-950 mb-1">Erreur lors de l'analyse</h2>
        <p className="text-xs text-rose-800 mb-6 font-semibold">{error}</p>
        <button
          onClick={loadStationData}
          className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-xs font-black text-white transition shadow-sm"
        >
          Réessayer
        </button>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <button
            onClick={onBack}
            className="p-2.5 rounded-xl bg-white border-2 border-slate-300 text-slate-800 hover:text-slate-950 hover:border-slate-400 transition shadow-xs cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4 stroke-[2.5]" />
          </button>
          <div>
            <div className="flex items-center gap-2 text-xs text-slate-700">
              <span className="font-mono font-black text-sky-900 bg-sky-100 px-2 py-0.5 rounded border border-sky-200">{reference}</span>
              <span className="font-black text-slate-400">•</span>
              <span className="font-bold text-slate-900">{sinistre.commune} ({sinistre.codePostal})</span>
              <span className="font-black text-slate-400">•</span>
              <span className="font-bold text-slate-900">{sinistre.dateSinistre || `${sinistre.dateDebut} au ${sinistre.dateFin}`}</span>
            </div>
      ...
      {/* BANDEAU CONSIGNE DE GESTION D'ASSURANCE - HAUTE VISIBILITÉ */}
      {insuranceDecision && (
        <div className={`p-5 rounded-2xl border-2 transition shadow-xs ${
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-xs ${
                ...
                {insuranceDecision.isFavorable ? <Check className="w-6 h-6 stroke-[3]" /> : <X className="w-6 h-6 stroke-[3]" />}
              </div>
              <div>
                <div className="flex items-center gap-2.5">
                  ...
                  <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-md border uppercase tracking-wider ${
                    insuranceDecision.isFavorable ? 'bg-emerald-200 text-emerald-950 border-emerald-300' : 'bg-rose-200 text-rose-950 border-rose-300'
                  }`}>
                    {insuranceDecision.isFavorable ? 'Garantie Acquise' : 'Garantie Non Acquise'}
                  </span>
                </div>
                <p className="text-xs font-bold mt-1 text-slate-950">
                  {insuranceDecision.ruleText} — <span className="text-slate-800 font-extrabold">{insuranceDecision.observedSummary}</span>
                </p>
              </div>
            </div>

            {/* Sélecteur de seuil contrat discret et contrasté */}
            <div className="flex items-center gap-2 bg-white px-3.5 py-2 rounded-xl border-2 border-slate-300 text-xs shadow-2xs">
              <Sliders className="w-4 h-4 text-sky-700" />
              <span className="text-slate-900 font-bold text-[11px]">Seuil contrat :</span>
              <select
      ...
      {/* BANDEAU DE VIGILANCE METEO-FRANCE HAUT CONTRASTE */}
      {liveVigilance && (
        <div className={`p-4 rounded-2xl border-2 shadow-2xs flex flex-wrap items-center justify-between gap-3 ${vigiConfig.bg}`}>
          <div className="flex items-center gap-3">
            <span className={`w-3.5 h-3.5 rounded-full ${vigiConfig.dot} shrink-0 ring-2 ring-white shadow-xs`}></span>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-black uppercase tracking-wide text-slate-950">
                  {vigiConfig.title}
                </span>
                <span className={`text-[10px] font-black px-2 py-0.5 rounded border uppercase ${vigiConfig.badge}`}>
                  Dép. {sinistre.codePostal?.slice(0, 2) || '59'}
                </span>
              </div>
              <p className="text-xs text-slate-800 font-medium mt-0.5">
                Statut officiel départemental Météo-France lors de l'événement. Relevés instrumentaux certifiés ci-dessous.
              </p>
            </div>
          </div>
          <span className="text-[10px] font-bold text-slate-800 bg-white px-3 py-1 rounded-lg border border-slate-300 shadow-2xs">
            Source Météo-France
          </span>
        </div>
      )}

      {/* 4 CARTES KPI METEO - TRÈS HAUTE VISIBILITÉ */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border-2 border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-black uppercase tracking-wider text-slate-800">Rafale Max</span>
            <Wind className="w-4 h-4 text-rose-600 stroke-[2.5]" />
          </div>
          {(() => {
            let maxFxi = null;
        ...
        <div className="bg-white rounded-2xl p-5 border-2 border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-black uppercase tracking-wider text-slate-800">Précipitations 24h</span>
            <Droplets className="w-4 h-4 text-sky-600 stroke-[2.5]" />
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border-2 border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-black uppercase tracking-wider text-slate-800">Températures</span>
            <Thermometer className="w-4 h-4 text-amber-600 stroke-[2.5]" />
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border-2 border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-black uppercase tracking-wider text-slate-800">Fiabilité</span>
      ...
      {/* TABLEAU COMPARATIF DES 5 STATIONS - HAUT CONTRASTE */}
              ...
              {stationsWithData.map((st, idx) => (
                <tr key={st.id || idx} className="hover:bg-slate-50 transition">
                  <td className="py-3.5 px-3 font-black text-slate-950 flex items-center gap-2">
                    ...
                    {idx === 0 && (
                      <span className="text-[9px] font-black text-sky-900 bg-sky-100 px-1.5 py-0.5 rounded border border-sky-300 uppercase">
                        Réf
                      </span>
                    )}
                  </td>
                  <td className="py-3.5 px-3 font-bold text-slate-900">{st.distance} km</td>
                  <td className="py-3.5 px-3 text-slate-700 font-mono font-bold">{st.alt || 0} m</td>
                  ...
                  <td className="py-3.5 px-3 text-slate-800 font-mono font-bold">
                    {st.obs?.hxi || '-'}
                  </td>
                  <td className="py-3.5 px-3 text-slate-950 font-bold font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-5 border-2 border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            <h2 className="text-xs font-black uppercase tracking-wider text-slate-950 mb-3 flex items-center gap-2">
              <FileText className="w-4 h-4 text-sky-600" />
              Synthèse de l'Expert Météorologue
            </h2>
            <div className="text-xs text-slate-900 leading-relaxed space-y-2 bg-slate-50 p-4 rounded-xl border border-slate-300 font-medium">
          ...
          <div className="mt-4 pt-3 border-t border-slate-200 flex items-center justify-between text-xs text-slate-600 font-bold">
            <span>Données certifiées Météo-France</span>
            <span className="font-mono text-sky-900">{reference}</span>
          </div>
        </div>
      </div>
                ...
                {activeTabStation.history.map((day, dIdx) => {
                  ...
                  return (
                    <tr key={day.date || dIdx} className="hover:bg-slate-50 transition">
                      ...
                      <td className="py-3 px-3 font-mono font-bold text-slate-700">{day.hxi || '-'}</td>
                      <td className="py-3 px-3 font-mono font-bold text-slate-950">{day.tn !== null ? `${day.tn}°` : '-'} / {day.tx !== null ? `${day.tx}°` : '-'}</td>
                      <td className="py-3 px-3 text-center">
                        <span className="text-slate-950 font-black text-xs bg-slate-100 px-3 py-1 rounded-md border border-slate-200 inline-block shadow-2xs">
                          {qualif}
                        </span>
                      </td>
                    </tr>
          ...
          return (
            <div className="mt-6 pt-5 border-t-2 border-slate-200">
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-950 mb-3 flex items-center gap-2">
                ...
                Records Historiques & Normales Météo-France — {activeTabStation.name} <span className="text-xs text-sky-900 font-mono font-bold bg-sky-50 px-2 py-0.5 rounded border border-sky-200">({activeTabStation.id})</span>
              </h3>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="p-4 rounded-2xl bg-white border-2 border-slate-300 shadow-2xs">
                  <span className="text-xs text-slate-800 uppercase font-black block">Record Absolu Vent</span>
                ...
                <div className="p-4 rounded-2xl bg-white border-2 border-slate-300 shadow-2xs">
                  <span className="text-xs text-slate-800 uppercase font-black block">Record Absolu Pluie 24h</span>
                ...
                <div className="p-4 rounded-2xl bg-white border-2 border-slate-300 shadow-2xs">
                  <span className="text-xs text-slate-800 uppercase font-black block">Normale Tn Mensuelle</span>
                ...
                <div className="p-4 rounded-2xl bg-white border-2 border-slate-300 shadow-2xs">
                  <span className="text-xs text-slate-800 uppercase font-black block">Normale Tx Mensuelle</span>
