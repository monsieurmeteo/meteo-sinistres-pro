    ...
    <div id="pdf-report-container" className="bg-slate-100 text-slate-900 font-sans hidden print:block">
      ...
      >
        ...
        <div className="space-y-3">
          ...
          {vigilanceStatus && (
            <div className={`border rounded-xl p-2.5 text-[8.5pt] flex items-center justify-between shadow-2xs ${vigiStyle?.bg}`}>
              <div className="flex items-center gap-2.5">
                <span className="text-xl">{vigiStyle?.icon}</span>
                <div>
          ...
          {isLightningClaim && (
            <div className="border border-sky-300 bg-sky-50/80 rounded-xl p-2.5 text-[8pt] flex items-center gap-2.5 text-sky-950">
          ...
          <div className="border border-slate-300 rounded-xl p-2.5 bg-white shadow-xs">
            <div className="flex justify-between items-center mb-1.5 px-1">
              ...
              <span className="text-[7.5pt] font-mono text-slate-500 font-bold">
            ...
            <div className="mt-2 text-center text-[8pt] text-slate-700 border-t border-slate-100 pt-1.5 font-medium">
        ...
        <div className="flex justify-between items-end pt-2.5 border-t border-slate-200 text-[8pt] text-slate-500">
          ...
          {qrUrl && (
            <div className="flex items-center gap-2">
              <div className="text-right">
                <span className="text-[7.5pt] font-bold text-slate-800 block">Vérifier ce rapport</span>
      ...
      >
        <div className="space-y-3.5">
          ...
          <div>
            ...
            <table className="w-full border-collapse border border-slate-300 text-[8.5pt] shadow-2xs rounded-lg overflow-hidden">
              <thead>
                <tr className="bg-slate-100 text-slate-900 font-black border-b-2 border-slate-300">
                  ...
                  <th className="border border-slate-300 p-2.5 text-center">Distance</th>
                  {showRain && <th className="border border-slate-300 p-2.5 text-center">Précipitations</th>}
                  {showWind && <th className="border border-slate-300 p-2.5 text-center">Rafale Max</th>}
                  {showWind && <th className="border border-slate-300 p-2.5 text-center">Heure Rafale</th>}
                  {showTemp && <th className="border border-slate-300 p-2.5 text-center">Tn Min</th>}
                  {showTemp && <th className="border border-slate-300 p-2.5 text-center">Tx Max</th>}
                </tr>
              </thead>
              <tbody>
                {activeStations.map((st, i) => (
                  <tr key={st.id || i} className={i === 0 ? 'bg-sky-50/70 font-semibold' : (i % 2 === 1 ? 'bg-slate-50/50' : '')}>
                    <td className="border border-slate-300 p-2.5">
                      ...
                      <span className="text-[7.5pt] text-slate-500 font-mono ml-1.5">({st.id})</span>
                      {i === 0 && (
                        <span className="text-[7pt] font-black text-sky-900 bg-sky-100 px-1.5 py-0.5 rounded ml-2 uppercase">
                          Station Principale
                        </span>
                      )}
                    </td>
                    <td className="border border-slate-300 p-2.5 text-center font-bold text-slate-800">{st.distance} km</td>
                    
                    {showRain && (
                      <td className="border border-slate-300 p-2.5 text-center text-cyan-900 font-bold">
                        {st.obs?.rr !== null && st.obs?.rr !== undefined ? `${st.obs.rr} mm` : 'N/D'}
                      </td>
                    )}

                    {showWind && (
                      <td className="border border-slate-300 p-2.5 text-center font-black text-rose-950">
                        {st.obs?.fxi !== null && st.obs?.fxi !== undefined ? `${st.obs.fxi} km/h` : 'N/D'}
                      </td>
                    )}

                    {showWind && (
                      <td className="border border-slate-300 p-2.5 text-center font-mono text-slate-700 font-medium">
                        {st.obs?.hxi || 'N/D'}
                      </td>
                    )}

                    {showTemp && (
                      <td className="border border-slate-300 p-2.5 text-center text-sky-900 font-medium">
                        {st.obs?.tn !== null && st.obs?.tn !== undefined ? `${st.obs.tn} °C` : 'N/D'}
                      </td>
                    )}

                    {showTemp && (
                      <td className="border border-slate-300 p-2.5 text-center text-amber-900 font-medium">
            ...
            <p className="text-[7pt] text-slate-500 mt-1 italic">
              * Mesures issues des capteurs certifiés OMM (anémomètre 10m, pluviomètre à auget basculeur, thermomètre sous abri ventilé).
            </p>
          </div>
          ...
          <div>
            ...
            <div className="border border-slate-200 rounded-xl p-3 bg-white text-[8.5pt] text-slate-800 leading-relaxed shadow-2xs">
              <p>
          ...
          <div>
            ...
            <div className="border border-slate-200 rounded-xl p-2.5 bg-slate-50/60 text-[7.5pt] text-slate-600 leading-relaxed">
              <p>
          ...
          <div>
            ...
            <div className="border border-slate-200 rounded-xl p-2.5 bg-white text-[7.5pt] text-slate-600 space-y-1 shadow-2xs">
